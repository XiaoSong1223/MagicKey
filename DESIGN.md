...
```
┌──────────────────────────────────────────────────────────┐
│  UI          SwiftUI + NSStatusItem/NSPopover 宿主        │
...
- **唯一的网络请求是更新检查**：向 GitHub Releases API 发一个匿名 GET，
  不带任何标识（无 UUID、无机器信息、无使用数据），暴露的信息量等同于
  用浏览器打开一次仓库页面。可在「设置 → 更新 → 自动检查更新」关闭，关掉后进程不联网。
- 除此之外无网络请求、无遥测、无崩溃上报
